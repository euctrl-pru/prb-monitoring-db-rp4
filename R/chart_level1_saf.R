...
myc <- function(
  ...
  fontsize = myfont,
  margin = 70,
  ansp_name = main_safety_ansp
) {
  plot_ly(
    data = filter(
      data_prep_maturity,
      tolower(entity_name) == tolower(ansp_name)
    ),
    width = width,
    height = height,
    x = ~year,
    ...
    add_annotations(
      text = c('Risk management target', 'Other MO targets'),
      x = 0.97,
      y = c(86, 66),
      showarrow = F,
      xref = "paper",
...
if (knitr::is_latex_output()) {
  myc(ansp_name = saf_ansps[[saf_ansp_index]], height = 290, fontsize = 12.5)
} else {
